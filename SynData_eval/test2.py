...
OUTPUT_ROOT = os.path.expanduser("/Users/pavankumartaddi/Desktop/llm-Drawing/result-exp")
...
# Modified Function Schema with only priority categories
FUNCTION_SCHEMA = {
...
def build_prompt() -> str:
    ...
    1. Categories (only generate these):
    ...
    - qa_pairs: 2-3 multiple-choice QA pairs
    ...
    Generate 2-3 new records now. Ensure:
...
def print_progress(counts: dict, targets: dict, total: int):
    """Updated progress display for priority categories only"""
...
def main():
    ...
    # Calculate targets evenly across priority categories
    category_targets = {
        cat: math.ceil(TARGET_COUNT / len(PRIORITY_CATEGORIES))
        for cat in PRIORITY_CATEGORIES
    }
